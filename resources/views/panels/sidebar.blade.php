    ...
    <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation" data-icon-style="lines">
      ...
      @if(!empty($menuData) && isset($menuData))
          ...
          @foreach ($menuData->menu as $menu)
              ...
              <li class="nav-item {{( Route::currentRouteName() === $menu->form_name ? 'active' :  (Request::segment(1) === $menu->form_name ? 'active' : ''  ) ) }}">

              <a href="@if(isset($menu->form_url)){{asset($menu->form_url)}} @endif" @if(isset($menu->newTab)){{"target=_blank"}}@endif>
                  @if(isset($menu->form_icon))
                      <i class="menu-livicon" data-icon="{{$menu->form_icon}}"></i>
                  @endif
                  @if(isset($menu->form_title))
                      <span class="menu-title text-truncate">{{ __('locale.'.$menu->form_title)}}</span>
                  @endif
